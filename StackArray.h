...
class StackArray {
  public:
    ...
    void pushAt(const T i, int index);
...
// push an item to the stack.
template<typename T>
void StackArray<T>::pushAt (const T i, int index) {
  // store the item to the array.
  contents[index] = i;
}
...
void StackArray<T>::blink () const {
  ...
  // while (true) {
  //   led = 1;                     // sets the LED on.
  //   wait_ms(250);                // pauses 1/4 of second.
  //   led = 0;                     // sets the LED off.
  //   wait_ms(250);                // pauses 1/4 of second.
  // }
